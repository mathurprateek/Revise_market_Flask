from market_flask import app, db
from flask import render_template, redirect, url_for, flash, request
from market_flask.models import Items, User
from market_flask.forms import RegisterForm, LoginForm, PurchaseItemForm, SellItemForm
    ...
    if request.method == 'POST':
        ...
        if p_item_object:
            if current_user.can_purchase(p_item_object.price):
                ...
                flash(f"Unfortunately, you do not have enough money to purchase {p_item_object.name}!",
                      category="danger")
...
def register_page():
    ...
    if form.errors != {}:
        for err in form.errors.values():
            flash(f"There was an error creating user: {err}", category='danger')
